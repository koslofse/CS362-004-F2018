...
int main() {
	...
	// set hand to have gold in odd positions and estates in even positions
	int i;
	...
	// last 2 hands cards should be gold
	if((testG.hand[0][testG.handCount[0]-1] == gold) && (testG.hand[0][testG.handCount[0]-2] == gold)){
		last2HandCardsAreGold = 1;
	}
	else{
		printf("FAIL - TEST 1: player's last 2 hands cards are not golds\n");
	}
